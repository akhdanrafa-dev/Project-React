...
const defaultCategoryId = catalogCategories.some(
...
const fallbackCategoryTheme =
    catalogCategories.find((category) => category.id === 'accessories') ??
    catalogCategories[0];
...
function normalizeProduct(
    ...
    const id = Number.isFinite(raw.id) ? Number(raw.id) : fallback.id;
...
function normalizeCategoryId(rawCategory: unknown): string {
    ...
    if (catalogCategories.some((category) => category.id === normalized)) {
        return normalized;
    }

    // Keep unknown categories from API so custom categories stay visible.
    return normalized || defaultCategoryId;
}

function getReadableCategoryName(categoryId: string): string {
    return categoryId
        .split('-')
        .filter(Boolean)
...
function createCategoryOption(id: string, name: string): CatalogCategory {
    const matchedStaticCategory = catalogCategories.find(
        (category) => category.id === id,
    );

    return {
        id,
        name: name.trim() || getReadableCategoryName(id),
        icon: matchedStaticCategory?.icon ?? fallbackCategoryTheme.icon,
        color: matchedStaticCategory?.color ?? fallbackCategoryTheme.color,
    };
}
...
function normalizeApiCategories(
    ...
    const response =
        payload && typeof payload === 'object'
            ? (payload as ApiCatalogResponse)
            : null;

    const categoryList = Array.isArray(response?.categories)
        ? (response?.categories as ApiCatalogCategory[])
        : [];

    const categoryMap = new Map<string, CatalogCategory>();

    categoryList.forEach((rawCategory) => {
        const id = normalizeCategoryId(rawCategory.slug ?? rawCategory.name);
        if (!id) return;

        const displayName =
            typeof rawCategory.name === 'string' && rawCategory.name.trim()
                ? rawCategory.name.trim()
                : getReadableCategoryName(id);

        categoryMap.set(id, createCategoryOption(id, displayName));
    });
...
export function CatalogProvider({ children }: { children: React.ReactNode }) {
    ...
    const refreshProducts = useCallback(async (signal?: AbortSignal) => {
        try {
            ...
            const nextProducts = normalizeApiProducts(payload);
            const nextCategories = normalizeApiCategories(
                payload,
                nextProducts,
            );
            ...
            console.error('Failed to sync catalog products from API:', error);
        }
    }, []);
    ...
    async function addProduct(product: CatalogProductInput) {
        ...
        const requestBody: Record<string, unknown> = {
            ...
            discount: Number.isFinite(product.discount)
                ? Math.min(100, Math.max(0, Number(product.discount)))
                : 0,
        ...
        const responsePayload: unknown = contentType.includes(
            'application/json',
        )
            ? await response.json()
        ...
        if (!response.ok) {
            ...
            const message =
                ...
                typeof (responsePayload as { message?: unknown }).message ===
                    'string'
        ...
        const createdProduct =
            ...
            typeof (responsePayload as { product?: unknown }).product ===
                'object' &&
        ...
        const normalizedCreated = normalizeProduct(
            {
                ...
                image:
                    typeof createdProduct?.image === 'string'
                        ...
                        : '',
                category: normalizeCategoryId(
                    createdProduct?.category_slug ?? createdProduct?.category,
                ),
            },
            fallback,
        );
    ...
    async function updateProduct(id: number, updates: Partial<CatalogProduct>) {
        const csrfToken = await ensureFreshCsrfToken();
        const requestBody: Record<string, unknown> = {};
        ...
        if (Number.isFinite(updates.stock)) {
            requestBody.stock = Math.max(0, Math.floor(Number(updates.stock)));
        }
        ...
        setProducts((prev) =>
            prev.map((product) => {
                ...
                const discountCandidate = Number.isFinite(updates.discount)
                    ? Math.min(100, Math.max(0, Number(updates.discount)))

...
export default function DeveloperStaffChat() {
    ...
    useEffect(() => {
        ...
        const loadStaffAndMessages = async () => {
            try {
                ...
                const match = staffData.users.find(
                    (u: StaffMember) => u.id === Number(staffId),
                );
                ...
                if (mounted && match) {
                    setStaff({
                        id: match.id,
                        name:
                            match.name ?? match.username ?? `User ${match.id}`,
                ...
                const messagesRes = await fetch(
                    `/api/staff-developer-chats/${staffId}/messages`,
                );
                if (messagesRes.ok) {
                    ...
                    if (mounted) {
                        const formattedMessages = (
                            ...
                            (msg: ChatMessageUI & { created_at?: string }) => ({
                                ...msg,
                                time: msg.created_at
                                    ? new Date(
                                          msg.created_at,
    ...
    const handleDeleteHistory = async () => {
        ...
        if (
            !window.confirm(
                'Apakah Anda yakin ingin menghapus riwayat obrolan? Tindakan ini tidak dapat dibatalkan.',
            )
        ) {
            return;
        }
        ...
        <RootLayout hideFloatingChat>
                ...
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <div className="flex items-center gap-2 text-xs text-muted-foreground">
                            <span
                                className={`h-2 w-2 rounded-full ${staff?.is_active ? 'bg-green-500' : 'bg-gray-400'}`}
                            />
                            <span>
                                {staff?.is_active
                                    ? 'Aktif sekarang'
                                    : `Dilihat ${staff?.last_seen ?? '-'}`}
                            </span>
                        </div>
                    </div>
                </div>
            ...
            <div className="flex h-[calc(100vh-4rem)] flex-col">
                <div className="flex-1 space-y-4 overflow-y-auto bg-muted/30 p-4">
                    {messages.length === 0 ? (
                        ...
                        messages.map((m) => (
                            <div
                                key={m.id}
                                className={`flex gap-3 ${m.sender === 'developer' ? 'justify-end' : 'justify-start'}`}
                            >
                                {m.sender === 'staff' && (
                                    <Avatar className="h-8 w-8">
                                        <AvatarFallback className="text-xs">
                                ...
                                <div className="max-w-[75%] space-y-1">
                                    ...
                                    <div
                                        className={`rounded-lg px-3 py-2 ${m.sender === 'developer' ? 'bg-primary text-primary-foreground' : 'border border-border bg-card text-card-foreground'}`}
                                    >
                                        {m.message}
                                    </div>
                ...
                >
                    ...
                    <Button
                        type="submit"
                        disabled={!staff || !text.trim() || isSending}
                    >

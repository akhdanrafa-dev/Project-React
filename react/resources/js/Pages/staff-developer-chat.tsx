    ...
    useEffect(() => {
        ...
        const loadDeveloperAndMessages = async () => {
            try {
                ...
                const match = developerData.users.find(
                    (u: Developer) => u.id === Number(developerId),
                );
                ...
                const messagesRes = await fetch(
                    `/api/staff-developer-chats/${developerId}/messages`,
                    {
                        credentials: 'same-origin',
                        headers: {
                            Accept: 'application/json',
        ...
        <RootLayout hideFloatingChat>
            ...
            <header className="flex h-16 items-center gap-2 border-b border-border bg-background px-4">
                ...
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <div className="flex items-center gap-2 text-xs text-muted-foreground">
                            <span
                                className={`h-2 w-2 rounded-full ${developer?.is_active ? 'bg-green-500' : 'bg-gray-400'}`}
                            />
                            <span>
                                {developer?.is_active
                                    ? 'Aktif sekarang'
                                    : `Dilihat ${developer?.last_seen ?? '-'}`}
                            </span>
                        </div>
                    </div>
                </div>
                <div className="ml-auto flex items-center gap-2">
                    {developer && (
                        <Badge
                            variant={
                                developer.is_active ? 'default' : 'secondary'
                            }
                        >
            ...
            <div className="flex h-[calc(100vh-4rem)] flex-col">
                ...
                >
                    ...
                    <Button
                        type="submit"
                        disabled={!developer || !text.trim() || isSending}
                    >
